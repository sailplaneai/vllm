...
class AutoModelForCausalLMWithValueHead(nn.Module):
    ...
    #                  revision: Optional[str] = None):
        self.pretrained_model.load_weights(weights)

        vhead_params = torch.load(osp.join(self.config._name_or_path, V_HEAD_WEIGHTS_NAME))
        vhead_params_without_prefix = {key.replace('v_head.', ''): value for key, value in vhead_params.items()}
        self.v_head.load_state_dict(vhead_params_without_prefix)
        # print()
        # params_dict = dict(self.v_head.named_parameters())
        # for name, loaded_weight in hf_model_weights_iterator(
        #         osp.join(model_name_or_path, V_HEAD_WEIGHTS_NAME), cache_dir, load_format, revision):
    ...
    def forward(
        ...
        hidden_states = self.pretrained_model.model(input_ids, positions, kv_caches,
    ...
    def compute_logits(self, hidden_states: torch.Tensor,
        ...
        logits = self.pretrained_model.logits_processor(self.pretrained_model.lm_head, hidden_states,
                                       sampling_metadata)
        return logits
    ...
    #     # next_tokens = self.pretrained_model.logits_processor(self.pretrained_model.lm_head.weight, hidden_states, input_metadata)  #  
