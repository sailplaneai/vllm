...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        if hasattr(self.model.config, "value_model") and model_input.is_prompt and self.model.config.value_model:
            values = self.model.v_head(hidden_or_intermediate_states)

            for idx, sequencegroupoutput in enumerate(output.outputs):
                ptr = model_input.sampling_metadata.selected_token_indices[idx]
                sequencegroupoutput.value_estimate = values[ptr].item()
